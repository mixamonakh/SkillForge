...
6. создать Evidence с provenance;
7. recompute affected TopicState и ReviewSchedule;
8. создать MetricSnapshot;
...
Если source bundle отсутствует или повреждён, explicit apply является безопасным no-op для knowledge state: batch получает `APPLIED` для audit/idempotency, snapshot фиксирует ноль affected topics, а Evaluation/Evidence/TopicState не создаются и не изменяются.
